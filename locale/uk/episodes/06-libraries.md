...
2. Виклик бібліотеки 3. Тут `sin` та `pi` посилаються на скорочену назву бібліотеки `m` замість `math`. Library call 3 does exactly that using the
  `import ... as ...` - створює псевдонім для `math` в формі короткого імені `m`.
3. Виклик бібліотеки 2. Here `sin` and `pi` are referred to with the regular library
